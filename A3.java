...
public class A3 extends JFrame {
	...
	class WidthActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (newValue > 0 && newValue < Shape.DEFAULT_MARGIN_WIDTH / 2) // if the value is valid, then change the
																				// current height
	...
	class HeightActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (newValue > 0 && newValue < Shape.DEFAULT_MARGIN_HEIGHT / 2) // if the value is valid, then change
																				// the current height
	...
	class AddActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			Shape newComp = (Shape) tree.getLastSelectedPathComponent();

			if (newComp instanceof NestedShape && newComp != null) {

				panel.addShapeNode((NestedShape) newComp);

			}

			else if (!(newComp instanceof NestedShape) && newComp != null) {
	...
	class RemoveActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			Shape selected = (Shape) tree.getLastSelectedPathComponent();

			if (selected != null) {

				if (!(panel.isRoot(selected))) {
					panel.removeNodeFromParent(selected);
				}
